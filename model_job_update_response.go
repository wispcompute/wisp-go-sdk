/*
Wisp API

Wisp API

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package wisp

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the JobUpdateResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &JobUpdateResponse{}

// JobUpdateResponse Job update response serializer.
type JobUpdateResponse struct {
	Message string `json:"message"`
}

type _JobUpdateResponse JobUpdateResponse

// NewJobUpdateResponse instantiates a new JobUpdateResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewJobUpdateResponse(message string) *JobUpdateResponse {
	this := JobUpdateResponse{}
	this.Message = message
	return &this
}

// NewJobUpdateResponseWithDefaults instantiates a new JobUpdateResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewJobUpdateResponseWithDefaults() *JobUpdateResponse {
	this := JobUpdateResponse{}
	return &this
}

// GetMessage returns the Message field value
func (o *JobUpdateResponse) GetMessage() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Message
}

// GetMessageOk returns a tuple with the Message field value
// and a boolean to check if the value has been set.
func (o *JobUpdateResponse) GetMessageOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Message, true
}

// SetMessage sets field value
func (o *JobUpdateResponse) SetMessage(v string) {
	o.Message = v
}

func (o JobUpdateResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o JobUpdateResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["message"] = o.Message
	return toSerialize, nil
}

func (o *JobUpdateResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"message",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varJobUpdateResponse := _JobUpdateResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varJobUpdateResponse)

	if err != nil {
		return err
	}

	*o = JobUpdateResponse(varJobUpdateResponse)

	return err
}

type NullableJobUpdateResponse struct {
	value *JobUpdateResponse
	isSet bool
}

func (v NullableJobUpdateResponse) Get() *JobUpdateResponse {
	return v.value
}

func (v *NullableJobUpdateResponse) Set(val *JobUpdateResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableJobUpdateResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableJobUpdateResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableJobUpdateResponse(val *JobUpdateResponse) *NullableJobUpdateResponse {
	return &NullableJobUpdateResponse{value: val, isSet: true}
}

func (v NullableJobUpdateResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableJobUpdateResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


